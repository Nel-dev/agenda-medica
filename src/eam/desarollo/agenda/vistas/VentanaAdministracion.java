...
public class VentanaAdministracion extends JInternalFrame {
	...
	private ArrayList<Profesional> pro;
	/**
	...
	public VentanaAdministracion() {
		...
		btnInformeProfesional.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					if (daoP.listaProfesional().size()==0) {
						...
						rowData[6] = "Direccion";
						model.addRow(rowData);
						pro=daoP.listaProfesional();
						
						for(Profesional p : pro){
							rowData[0] = Integer.valueOf(p.getIdprofesional());
							rowData[1] = Integer.valueOf(p.getIdentificacionProfesional());
							rowData[2] = p.getNombreProfesional();
							rowData[3] = p.getApellidoProfesional();
							rowData[4] = p.getAreaProfesional();
							rowData[5] = p.getTelefonoProfesional();
							rowData[6] = p.getDireccionProfesional();
							model.addRow(rowData);
						}
						table = new JTable(model);
						table.getColumnModel().getColumn(0).setCellRenderer(table.getTableHeader().getDefaultRenderer());
